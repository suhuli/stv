...
// PASSWORD is not required; proxy access is public.
// --- 配置结束 ---
...
export async function onRequest(context) {
    ...
    async function fetchContentWithType(targetUrl) {
        ...
        try {
            ...
            const isBinary = MEDIA_CONTENT_TYPES.some(type => lowerContentType.startsWith(type)) ||
                MEDIA_FILE_EXTENSIONS.some(extension => pathname.endsWith(extension));

            // 图片、音视频等资源必须按二进制读取，否则 UTF-8 解码会损坏文件。
    ...
    async function processMasterPlaylist(url, content, recursionDepth, env) {
        ...
        try {
            kvNamespace = env.STV_PROXY_KV; // 从环境获取 KV 命名空间 (变量名在 Cloudflare 设置)
            if (!kvNamespace) throw new Error("KV 命名空间未绑定");
        } catch (e) {
            logDebug(`KV 命名空间 'STV_PROXY_KV' 访问出错或未绑定: ${e.message}`);
    ...
    try {
        ...
        // --- 缓存检查 (KV) ---
        // v2 prevents reuse of entries written by the old text-only implementation.
        const cacheKey = `proxy_raw:v2:${targetUrl}`;
        let kvNamespace = null;
        try {
            kvNamespace = env.STV_PROXY_KV;
            if (!kvNamespace) throw new Error("KV 命名空间未绑定");
        } catch (e) {
            logDebug(`KV 命名空间 'STV_PROXY_KV' 访问出错或未绑定: ${e.message}`);
            kvNamespace = null;
