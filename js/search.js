async function searchByAPIAndKeyWord(apiId, query, externalSignal) {
    try {
        ...
        const { controller, onAbort } = createLinkedController();
        ...
        // 如果有额外页数且未被取消，获取更多页的结果
        if (pagesToFetch > 0 && !(externalSignal && externalSignal.aborted)) {
            ...
            for (let page = 2; page <= pagesToFetch + 1; page++) {
                ...
                const pagePromise = (async () => {
                    try {
                        const { controller: pageController, onAbort: onPageAbort } = createLinkedController();
                        ...
                        if (externalSignal) externalSignal.removeEventListener('abort', onPageAbort);
