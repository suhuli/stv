...
function fillAndSearch(title) {
    ...
    if (input) {
        ...
        try {
            ...
            // 更新页面标题
            document.title = `搜索: ${safeTitle} - 私人TV`;
        } catch (e) {
...
function renderDoubanCards(data, container) {
    ...
    if (!data.subjects || data.subjects.length === 0) {
        ...
        data.subjects.forEach(item => {
            ...
            // 2. 也准备代理URL作为备选
            // Version the fallback URL so browsers and edge caches skip pre-fix corrupted responses.
            const proxiedCoverUrl = PROXY_URL + encodeURIComponent(originalCoverUrl) + '?v=2';
